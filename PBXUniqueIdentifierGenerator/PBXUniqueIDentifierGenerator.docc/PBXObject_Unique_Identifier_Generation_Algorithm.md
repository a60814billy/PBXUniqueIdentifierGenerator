...
| **Random** | 32 bits (4 bytes) | A random 32-bit integer generated once per session. |
...
*   **Random Seed**: A 32-bit random value is generated using `arc4random()`. This value remains constant for all IDs generated by this process instance.
    > **Note**: The original Xcode implementation used `hostid` and the current time to seed the random number generator. This implementation simplifies this by using `arc4random()` directly.
